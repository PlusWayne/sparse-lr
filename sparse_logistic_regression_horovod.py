...
class SparseLogisticRegression(object):
    ...
    def train_step(self, sparse_ids, sparse_vals, label, optimizer,first_batch=0):
        with tf.GradientTape() as tape:
        ...
        # initialization.
        if first_batch:
            hvd.broadcast_variables([self.w, self.b], root_rank=0)
    ...
    def fit(self, train_id, train_val, label):
        ...
        for i in range(self.iteration):
            if i==0:
                self.train_step(sparse_ids, sparse_vals, label, optimizer,first_batch=1)
            else:
                self.train_step(sparse_ids, sparse_vals, label, optimizer, first_batch=0)
            if i % 100 == 0:
...
if __name__ == '__main__':
    ...
    """test distributed sparse logistic regression"""
